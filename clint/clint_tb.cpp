...
int main(int argc, char **argv, char **env) {
    ...
    write(dut, tfp, 0x0, 1);
    if (dut->msi_o != 1)
        printf("ERROR: MSI not asserted after writing 1 to MSIP\n");
    else
        printf("MSI asserted correctly\n");

    write(dut, tfp, 0x0, 0);
    if (dut->msi_o != 0)
        printf("ERROR: MSI not de-asserted after writing 0 to MSIP\n");
    else
        printf("MSI de-asserted correctly\n");
    ...
    if (dut->mti_o != 1)
        printf("ERROR: MTI not asserted after timeout\n");
    else
        printf("MTI asserted correctly\n");
    ...
    if (dut->mti_o != 0)
        printf("ERROR: MTI not de-asserted after increasing MTIMECMP\n");
    else
        printf("MTI de-asserted correctly\n");
