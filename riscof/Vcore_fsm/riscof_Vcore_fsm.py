...
class Vcore_fsm(pluginTemplate):
    __model__ = "Vcore_fsm"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "/home/chvsnaveen12/cores/linux-core/core/obj_dir","Vcore_fsm")
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       self.objcopy_cmd = 'riscv64-unknown-elf-objcopy -O binary {0} {1}'
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for emu start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    #         simcmd = self.dut_exe + '{2}'.format(elf, sig_file)
    #       else:
    #         simcmd = 'echo "NO RUN"'
...
#The following template only uses shell commands to compile and run the tests.
    ...
    def runTests(self, testList):
     ...
     for testname in testList:
         ...
         cmd = self.objcopy_cmd.format(elf, bin, self.xlen)
         utils.shellCommand(cmd).run(cwd=test_dir)

         sig_begin = subprocess.check_output(['nm my.elf | grep "begin_signature" | egrep "[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]" -o'], cwd=test_dir, text=True, shell=True)
         sig_end = subprocess.check_output(['nm my.elf | grep "end_signature" | egrep "[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]" -o'], cwd=test_dir, text=True, shell= True)
        #  print(sig_begin)
        #  print(sig_end)
        ...
        #    execute = self.dut_exe + '{0} +signature={1} +signature-granularity=4 {2}'.format(self.isa, sig_file, elf)
           execute = self.dut_exe + " " + bin + " " + sig_begin.strip() + " " + sig_end.strip() + ' > ' + sig_file;
