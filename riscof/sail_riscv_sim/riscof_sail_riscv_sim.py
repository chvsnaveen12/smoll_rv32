...
class sail_riscv_sim(pluginTemplate):
    __model__ = "sail_riscv_sim"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ref_exe = os.path.join(config['PATH'] if 'PATH' in config else "","sail_riscv_sim")
        self.num_jobs = str(config['jobs'] if 'jobs' in config else 1)
        self.pluginpath=os.path.abspath(config['pluginpath'])
        self.isa_spec = os.path.abspath(config['ispec']) if 'ispec' in config else ''
        self.platform_spec = os.path.abspath(config['pspec']) if 'ispec' in config else ''
        self.make = config['make'] if 'make' in config else 'make'
        logger.debug("sail_riscv_sim plugin initialised using the following configuration.")
        for entry in config:
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            execute += self.ref_exe + ' --config /home/chvsnaveen12/uc_speedrun/riscof/rv32_config.json --test-signature={0} {1} > {2}.log 2>&1;'.format(sig_file, elf, test_name)
